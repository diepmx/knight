...
public class Water : Weapon
{
    ...
    // Tham chi?u ??n prefab ??i di?n cho tia n??c ho?c bong bóng n??c.
    public ProjectileWeapon waterProjectile;
    ...
    void Awake()
    {
        // ??m b?o r?ng ch? có m?t instance duy nh?t c?a Water.
        instance = this;
    }
    ...
    void Update()
    {
        ...
        if (shotCounter <= 0)
        {
            ...
            if (enemies.Length > 0)
            {
                ...
                {
                    // Ch?n ng?u nhiên m?t k? thù trong ph?m vi và t?o ra tia n??c.
                    Vector3 targetPosition = enemies[Random.Range(0, enemies.Length)].transform.position;
                    Instantiate(waterProjectile, targetPosition, Quaternion.identity).gameObject.SetActive(true);
                }
        ...
        // C?p nh?t c?p ?? c?a Water.
        waterLevel = weaponLevel;
    }

    // C?p nh?t các ch? s? v? khí khi c?p ?? thay ??i.
    void SetStats()
    {
        // C?p nh?t các ch? s? c?a damager (tia n??c).
